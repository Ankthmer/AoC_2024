...
def occurrences(p: str) -> int:
    ...
    for i in range(7, len(p)):
        try:
            #If you find ')' and you find that the previous one is a number go backward
            if p[i] == ')' and p[i-1].isnumeric():
                for j in range(i-1, -1, -1):
                    if p[j].isnumeric():
                        pass
                    #and if after some numbers you find ',' and that the previous one is a number save the number between ',' and ')'. Then go backward
                    elif p[j] == ',' and p[j-1].isnumeric():
                            a = int(p[j+1:i])
                            for m in range(j-1, -1, -1):
                                if p[m].isnumeric():
                                    pass
                                #and if after some numbers you find 'mul(' save the number between '(' and ',' and update your counter
                                elif p[m-3 : m+1] == 'mul(':
                                    b = int(p[m+1:j])
                                    counter += a*b
                                    break
